...
**如果你的服务器已经在 `~/.ssh/config` 中配置好了别名**，可以只写服务器名称：
...
系统会自动从 `~/.ssh/config` 中读取对应主机的 `HostName`、`User`、`Port`、`IdentityFile` 等参数，无需重复填写。连接参数完全由 `~/.ssh/config` 管理，你的配置文件只负责定义名称。

如果某个服务器需要额外的配置（如命令白名单），可以在对象中添加：
